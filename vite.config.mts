...
export default defineConfig(({ mode }) => {
  ...
  const backendPort = parsePort(env.AIONUI_BACKEND_PORT || env.AIONUI_PORT, 25808);
  const backendHost = env.AIONUI_BACKEND_HOST || '127.0.0.1';
  ...
  if (wsPath) {
    proxy[wsPath] = { target: `ws://${backendHost}:${backendPort}`, changeOrigin: true, ws: true };
  }
