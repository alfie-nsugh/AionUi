...
export default defineConfig({
  ...
  rules: [
    ...
    [/^text-([1-4])$/, ([, d]: RegExpMatchArray) => ({ color: `var(--color-text-${d})` })],

    // Arco Design 官方填充色 fill-1 到 fill-4
    // Arco Design official fill colors: bg-fill-1, bg-fill-2, bg-fill-3, bg-fill-4
    [/^bg-fill-([1-4])$/, ([, d]: RegExpMatchArray) => ({ 'background-color': `var(--color-fill-${d})` })],
    ...
    [/^border-arco-([1-4])$/, ([, d]: RegExpMatchArray) => ({ 'border-color': `var(--color-border-${d})` })],
    ...
    [/^bg-(primary|success|warning|danger|link)-light-([1-4])$/, ([, color, d]: RegExpMatchArray) => ({ 'background-color': `var(--color-${color}-light-${d})` })],
    ...
    [
      ...
      ([, prefix, color, d]: RegExpMatchArray) => {
