...
// These constants are injected by Electron Forge plugins (Webpack/Vite) at build time.
...
const createWindow = (): void => {
  ...
  const preloadPath = typeof MAIN_WINDOW_PRELOAD_WEBPACK_ENTRY !== 'undefined' && MAIN_WINDOW_PRELOAD_WEBPACK_ENTRY ? MAIN_WINDOW_PRELOAD_WEBPACK_ENTRY : path.join(__dirname, '../preload/index.js');
  ...
  const hasWebpackEntry = typeof MAIN_WINDOW_WEBPACK_ENTRY !== 'undefined' && Boolean(MAIN_WINDOW_WEBPACK_ENTRY);
  const rendererName = typeof MAIN_WINDOW_VITE_NAME !== 'undefined' && MAIN_WINDOW_VITE_NAME ? MAIN_WINDOW_VITE_NAME : 'main_window';

  const loadPromise = viteDevServerUrl ? mainWindow.loadURL(viteDevServerUrl) : hasWebpackEntry ? mainWindow.loadURL(MAIN_WINDOW_WEBPACK_ENTRY as string) : mainWindow.loadFile(path.join(__dirname, `../renderer/${rendererName}/index.html`));

  loadPromise.catch((_error) => {
